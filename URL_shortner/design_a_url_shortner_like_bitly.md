...
We choose NoSQL here assuming that we will require a very high write throughput and scale. We will go with Cassandra.
...
But it too has problems
    ...
    - The collision rate is very high over here if we are going to generate 1000 short URLs per second. The time stamp will not change, the 4-Bit substring of the Mac address will also not change, and the 6-bit counter can have collisions if generated by different worker threads in the same second

3. Range Based using Zookeeper: We can use a distributed coordination service - Zookeeper to coordinate amongst the different instances of Counter services.
...
3. The Zookeeper cluster is preconfigured with a range of Unique ID values to be alloted to each instance of service. We will split the 3.5 trillion total number of combinations into below ranges each of size 1 million each.

       S1       1  to 1,00,0000
       S2       1,00,0001 to 2,00,0000
       S3       2,00,0001 to 3,00,0000
       S4       3,00,0001 to 4,00,0000
